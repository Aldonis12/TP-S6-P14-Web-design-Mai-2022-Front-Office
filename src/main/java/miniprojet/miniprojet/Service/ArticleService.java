...
public class ArticleService {
    ...
    public List<Article> AllArticle(){
        List<Article> liste = dao.findQuery(Article.class,"Select * from article where datePublication < now()");
        return liste;
    }
    ...
    public List<Actualite> Actualite(){
        ...
        for (Object[] result : results) {
            ...
            if (date instanceof Timestamp) {
                ...
                String formattedDate = localDateTime.format(DateTimeFormatter.ofPattern("EEEE d MMMM yyyy à HH:mm:ss", Locale.FRENCH));
                article.setTitre(formattedDate);
            }
            actualite.setArticle(article);
            Timestamp temps = (Timestamp) result[2];
            actualite.setDateCreation(GetTempsApres(temps.toLocalDateTime()).toString());
